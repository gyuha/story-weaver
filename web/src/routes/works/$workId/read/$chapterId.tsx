...
export const Route = createFileRoute('/works/$workId/read/$chapterId')({
  beforeLoad: ({ params }) => {
    ...
    // chapters가 이미 로드된 경우에만 잘못된 챕터 → 기본(첫) 챕터로 fast-path 리다이렉트.
    // 딥링크 등으로 chapters가 아직 []인 경우는 통과시켜 컴포넌트의 하이드레이션에 맡긴다.
    if (work && work.chapters.length > 0 && !work.chapters.some((c) => c.id === params.chapterId)) {
      throw redirect({
...
export function ReadPage() {
  ...
  // 하이드레이션 완료 후에도 챕터를 찾지 못하면 기본(첫) 챕터로, 챕터가 없으면 집필로 교정.
  // 렌더 클로저의 work/nav는 setWorkChapters가 같은 커밋에서 반영한 chapters를 못 볼 수 있어
  // 스토어를 직접 읽는다(WorkLayout의 하이드레이션 경합 수정과 동일한 이유).
  useEffect(() => {
    if (isPending || isError || nav) return;
    const fresh = useWorksStore.getState().works.find((w) => w.id === workId);
    if (!fresh) return;
    // 스토어에는 이미 유효한 챕터가 반영됐고 리렌더만 남은 상태면 교정 불필요
    if (fresh.chapters.some((c) => c.id === chapterId)) return;
    const fallback = fresh.chapters[0]?.id;
    if (fallback) {
      navigate({
        to: '/works/$workId/read/$chapterId',
        params: { workId, chapterId: fallback },
        replace: true,
      });
    } else {
      navigate({ to: '/works/$workId/write', params: { workId }, replace: true });
    }
  }, [isPending, isError, nav, workId, chapterId, navigate]);

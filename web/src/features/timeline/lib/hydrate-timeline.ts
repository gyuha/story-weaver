...
// 엔티티별 타임라인 상태를 조회해 웹 mock 모양(TimelineState[])으로 매핑한다. chapterRef('6화')는
// 화 목록에서 chapterId 위치를 찾아 조립 — eco: fetchWorkChapters를 여기서 별도 재조회한다
  ...
  const perEntity = await Promise.all(
    entities.map(async (entity) => {
      ...
      return states.map((state) => ({
        state: toTimelineState(state, entity.name, chapterRefs.get(state.chapterId)),

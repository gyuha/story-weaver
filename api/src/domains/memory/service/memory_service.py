...
청킹 임베딩으로 대체). 로컬 임베딩 클라이언트(``embedding_client.aembed_text``)는
API 키·비용 없이 별도 큐 없이 호출 지점에서 바로 처리하되, 모델 인퍼런스 자체가
동기·CPU 바운드라 이벤트 루프를 막지 않도록 별도 스레드에서 실행한다(plan.md S2).
...
def _chunk_paragraphs(body: str, max_chars: int = _CHAPTER_CHUNK_MAX_CHARS) -> list[str]:
    ...
    분할은 비목표 — plan.md S2). 문단이 하나도 안 나오면 본문이 공백뿐이라는 뜻이므로
    청크 0개를 반환한다(빈 문자열 임베딩 저장 방지).
    """
...
class MemoryService:
    ...
    async def index_source(
        ...
        if existing is not None and existing.content == content:
            return existing  # eco: 내용 불변 — aembed_text() 스킵(비용이 드는 부분)
        vector = await aembed_text(content)

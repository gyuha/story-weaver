...
타임라인 상태는 ``work_id``+``entity_id``로 스코프하고, ``up_to_scene_id`` 필터는
manuscript 도메인이 계산해 넘겨준 "그 이하 global_seq를 가진 씬 id 목록"으로 적용한다
(scenes 테이블을 직접 조회/조인하지 않아 도메인 간 직접 모델 import 없이 ID만으로
필터, timeline_service.py 참조). 씬-엔티티 링크는 ``scene_id``+``entity_id``로 직접
...
class TimelineRepository:
    ...
    async def list_links(self, scene_id: uuid.UUID) -> list[SceneEntityLink]:
        result = await self._session.execute(
            select(SceneEntityLink)
            .where(SceneEntityLink.scene_id == scene_id)
            .order_by(SceneEntityLink.created_at)
        )
        return list(result.scalars().all())

    async def get_link(self, scene_id: uuid.UUID, entity_id: uuid.UUID) -> SceneEntityLink | None:
        result = await self._session.execute(
            select(SceneEntityLink).where(
                SceneEntityLink.scene_id == scene_id, SceneEntityLink.entity_id == entity_id
            )
        )

"""manuscript 모델 실 DB 테스트 — 3개 테이블 insert + FK cascade 확인.

실 `core.database.AsyncSessionFactory`로 시놉시스·부(episode)·챕터를 각 1건씩
insert하고, 부 삭제가 챕터까지 DB FK cascade로 지워지는지 확인한다
(works 도메인 test_works_isolation.py의 실 DB 테스트 패턴을 따름). 챕터가

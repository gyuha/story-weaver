...
> AI 웹소설 창작 SaaS "StoryWeaver"의 백엔드 — auth(JWT+OAuth+RBAC)와 LLM 집필 프록시 도메인.
...
**StoryWeaver API** 는 AI 웹소설 창작 SaaS의 FastAPI 기반 백엔드 서버입니다. (설계 문서는 저장소 루트의 `docs/` 참조.)

- 생성 메타데이터: project slug `story-weaver-api`, public API base `http://localhost:8000`.
...
| `MAIL_FROM` | `noreply@storyweaver.example.com` | 발신 이메일 주소 |

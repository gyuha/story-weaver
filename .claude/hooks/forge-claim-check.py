...
6. 방어를 깨서 red를 확인했더라도, **그 테스트가 보는 명제가 목표와 같은지** 물으십시오.
   "red가 된다"는 "옳은 것을 검증한다"를 보장하지 않습니다. #71에서 `zIndex: 60`을 주고
   렌더된 style을 단정하는 테스트로 red까지 확인했지만, 실제 원인은 stacking context라
   화면은 그대로였습니다 — 테스트는 "style에 값이 붙는다"를 정확히 증명했고 그건
   목표("위에 그려진다")가 아니었습니다.
   그리고 **jsdom이 관측할 수 없는 것**(페인팅·레이아웃·쌓임 순서·실제 크기)을
   테스트로 증명했다고 적지 마십시오. 그건 브라우저에서만 확인됩니다.
{extra}"""
...
def main() -> int:
    ...
    if path.endswith(("plan.md",)) or "/backlog/" in path:
        ...
        if content and _VERIFY_SECTION not in content:
            extra = (
                f"\n7. 이 계획서에 '{_VERIFY_SECTION}' 절이 없습니다. 직전 회고의 "

...
> 본 문서는 ADR-0002(하이브리드 메모리: 정형 엔티티 카드 + 타임라인 상태 + 벡터(pgvector) + 화-엔티티 링크)를 데이터 스키마로 구체화한다. 용어는 `.forge/CONTEXT.md`의 canonical 정의를 따른다. 기능 범위는 `docs/PRD.md`를 따른다 — **MVP에서 타임라인 상태는 기록·표시까지이고, 자동 설정 충돌 감지는 v2**다.
>
> **Scene 계층은 제거됐다(remove-scene ADR, 260716-17a)** — 챕터(Chapter)가 본문(`body`)을 직접 보유하는 집필·메모리·AI 생성의 최소 단위다. "씬-엔티티 링크"는 개념상 "화-엔티티 링크"로 개정됐으나, 테이블명(`scene_entity_links`)·모델 클래스명(`SceneEntityLink`)은 코드 변경 범위를 좁히기 위해 그대로 유지한다 — FK 컬럼만 `chapter_id`로 바뀌었다.
>
...
1. **계층 (Hierarchy):** 작품(Work) → 시놉시스(Synopsis) → 부(Part) → 챕터(Chapter). 원고의 구조적 뼈대 — 챕터가 본문(`body`)을 직접 보유하는 최소 단위다.
2. **설정 (World Bible):** 엔티티 카드(Entity Card) — 인물·장소·사건·아이템. 작품의 "기억" 원천 데이터.
3. **상태/링크 (State & Link):** 타임라인 상태(Timeline State)와 화-엔티티 링크(테이블명 `scene_entity_links` 유지). "언제 무엇이 어떻게 변했는가"와 "어느 화에 누가 나오는가"를 잇는다.
4. **벡터 (Embedding):** 엔티티 카드와 화 본문(문단 그룹핑 청킹)을 임베딩해 의미 검색을 보조한다(메모리의 보조 근거).
...
사용자 계정(users)
   └─ 작품(Work)                         ← 멀티테넌시 격리 경계
        ...
        ├─ 화-엔티티 링크(scene_entity_links)  ← 화 ↔ 엔티티 다대다
        └─ 임베딩(Embedding)             ← 카드/화 청크의 벡터
```

---

## 2. 계층 모델 (Work → Synopsis → Part → Chapter)
...
작품은 시놉시스 1개와 다수의 부를 가진다. 부는 다수의 챕터를 가진다. **챕터(Chapter)** 는 집필과 AI 생성의 최소 단위이며 실제 원고 본문(`body`)을 보유하는 유일한 계층이다(remove-scene ADR — 이전에는 챕터 아래 씬(Scene)이 최소 단위였으나 그 계층을 제거하고 챕터로 흡수했다). 상위 계층(부)은 구조·메타데이터만 보유한다.

순서가 의미를 갖는 계층(부·챕터)은 `order_index`로 형제 간 순서를 표현한다. (드래그 앤 드롭 순서 변경 UI는 v2 Plot Architect 소관이나, 순서 필드 자체는 MVP에서 존재한다.)

타임라인 상태와 충돌 감지가 **챕터 시점**을 기준으로 동작하므로(4장), 챕터는 작품 전체에서 단조 증가하는 정렬 가능한 시점값을 가져야 한다. 이를 위해 챕터에 `global_seq`(작품 내 전역 순서)를 둔다 — "3화에서 사망 < 10화에서 등장" 같은 시점 비교의 근거다.
...
| `summary` | text NULL | **화별 줄거리 요약** — "이 화에서 무슨 일이 일어났는가" 서술. `검토 · 타임라인` 화면이 화 순서대로 모아 보여준다. 아직 요약하지 않은 화는 NULL. 엔티티 카드의 `summary`(3.1)와 이름만 같고 다른 개념 — 이쪽은 임베딩하지 않는다 |

> **`global_seq` 재계산:** 챕터 삽입·이동 시 작품 내 전역 순서를 다시 부여해야 한다. 재계산을 매 변경마다 즉시 할지, 조회 시 `(episode.order_index, chapter.order_index)` 정렬로 대체할지는 **미결정**(성능 슬라이스에서 확정). 본 모델은 "정렬 가능한 시점값이 존재한다"는 불변식만 요구한다.
...
> `name`/`aliases`는 동적 업데이트 제안과 화-엔티티 링크 자동 추출에서 본문 내 등장 매칭의 1차 근거다. 매칭 알고리즘 자체(정확 일치/형태소/임베딩)는 메모리 파이프라인 설계 슬라이스 소관 — 본 문서는 **미결정**.
...
**사건 (`event`)**: `description`, `participants`(관련 엔티티 id[]), `occurred_at_scene`(발생 시점 챕터 id, nullable — 필드명은 레거시로 `scene`을 유지).
...
타임라인 상태는 **한 엔티티가 특정 시점(챕터)에 갖는 상태**를 기록하는 모델이다. 엔티티 카드(3장)가 "현재/요약 설정"이라면, 타임라인 상태는 "그 설정이 언제 어떻게 바뀌었는가"의 시점별 기록이다. 이것이 제품의 핵심 차별점인 상태/사실 추적과 (v2) 설정 충돌 감지의 토대 데이터다(ADR-0002).

핵심 설계 원칙: **상태는 시점(챕터)에 묶인 불변 사실의 누적**이다. 카드의 `attributes`를 덮어쓰는 대신, "화 N 시점에 이 엔티티는 이런 상태였다"를 한 행씩 쌓는다. 시점 비교는 챕터의 `global_seq`(2장)로 한다.
...
| `chapter_id` | UUID FK → chapters | **상태가 확정된 시점 챕터.** 시점 비교는 이 챕터의 `global_seq` |
...
AI 동적 업데이트 제안 (source=ai_suggested)
        ↓ 작가 승인
타임라인 상태 1행 추가 (entity_id, chapter_id, state_key, state_value)
        ↓
...
## 5. 화-엔티티 링크 (테이블명 `scene_entity_links` 유지)

화-엔티티 링크는 **특정 화에 어떤 엔티티가 등장하는지**를 명시하는 다대다 연결이다. 메모리가 무엇을 불러올지 정하는 **1차 근거**이고, 벡터 유사도는 보조다(ADR-0002, CONTEXT). 한 화는 여러 엔티티를, 한 엔티티는 여러 화를 가질 수 있다.

> remove-scene ADR(260716-17a) 이전에는 씬-엔티티 링크였다. 개념은 화 단위로 개정됐지만 테이블명(`scene_entity_links`)과 ORM 클래스명(`SceneEntityLink`)은 변경 범위를 좁히기 위해 그대로 유지한다 — FK 컬럼만 `scene_id`→`chapter_id`로 바뀌었다.
...
| `created_at` | timestamptz | |
| | | **UNIQUE(`chapter_id`, `entity_id`)** — 동일 쌍 중복 방지 |

링크 생성 경로는 두 가지다: 작가 수동 연결(`author`)과, 화 본문에서 엔티티 `name`/`aliases`를 매칭해 AI가 제안하는 자동 추출(`ai_extracted`). 자동 추출의 매칭/제안 정밀도와 작가 확인 UX는 메모리 파이프라인 슬라이스 소관 — **미결정**.

### 5.2. 메모리 조회 흐름

현재 화를 집필할 때, 메모리는 먼저 화-엔티티 링크로 명시 등장 엔티티를 가져오고(1차), 그것으로 부족한 관련 설정을 벡터 검색으로 보충한다(보조). 두 경로 모두 `work_id`로 스코프되어 타 테넌트 데이터를 절대 반환하지 않는다(7장).

```mermaid
flowchart TD
    S[현재 집필 화] --> L[화-엔티티 링크 조회<br/>명시 등장 엔티티 - 1차]
    S --> Q[화 본문 청크 임베딩 → 벡터 유사도 검색<br/>관련 설정 보충 - 보조]
...
벡터는 메모리의 **보조** 경로다. 무엇을 임베딩하는가: (a) 엔티티 카드의 `summary` + 주요 `attributes` 텍스트, (b) 화 본문(`chapters.body`). 화 본문은 길어질 수 있으므로 문단 그룹핑(목표 ~800자)으로 청킹 후 청크 단위로 임베딩한다(`chunk_index 0..N-1`, remove-scene ADR). 엔티티 카드는 통상 짧아 카드당 1청크로 시작한다(길면 분할).
...
| `source_type` | enum(`entity`,`chapter`) | 원본 종류 |
| `source_id` | UUID | 원본 엔티티 또는 챕터 id |
...
엔티티 카드나 화 본문이 변경되면 해당 `source_id`의 임베딩을 무효화·재생성해야 한다. 화는 재청킹 결과 청크 수가 줄면 남는 뒤쪽 `chunk_index` 행을 정리한다. 동기/비동기(큐) 처리 여부는 **미결정**.

```
엔티티 카드/화 본문 변경 → 해당 source_id 임베딩 무효화 → (화는 재)청킹 → 재임베딩 → embeddings 갱신
```
...
상용 SaaS이므로 테넌트(사용자 계정) 간 데이터 격리는 필수다(PRD 4.5). 소유 사슬은 `embeddings/entities/chapters/... → works → users`다. 핵심 불변식: **모든 도메인 테이블이 `work_id`를 직접 보유하고, 모든 쿼리는 `work_id`(궁극적으로 `user_id`)로 스코프된다.** 격리 enforcement는 애플리케이션 레이어 스코핑(now) + RLS(후속)다(ADR-0005). 메모리·벡터 검색 결과가 타 테넌트 데이터를 반환하지 않도록 한다.
...
`episodes`/`chapters`/`entities`/`timeline_states`/`scene_entity_links`/`embeddings` 모두 `work_id`를 직접 가진다. 계층을 타고 조인하면 작품을 알 수 있음에도 비정규화하는 이유: 모든 격리 쿼리(특히 벡터 검색)가 조인 없이 단일 컬럼으로 선필터되게 하여, 격리 누락 실수(조인 빠뜨림)를 구조적으로 막기 위함이다. 이는 의도된 중복이다.
...
1. **3화 사망 기록 (타임라인 상태):**
   - 3화(챕터 `C3`, 예: `global_seq = 3`)에서 김무사가 죽는 장면을 집필.
   - AI 동적 업데이트 제안 → 작가 승인 → `timeline_states`에 1행 추가:
     `{ work_id: W, entity_id: E, chapter_id: C3, state_key: "life_status", state_value: "dead", source: "ai_suggested" }`
2. **10화 등장 (화-엔티티 링크):**
   - 10화(챕터 `C10`, 예: `global_seq = 10`)에서 김무사가 대사를 하는 장면을 집필.
   - 본문에 "김무사"가 등장 → `scene_entity_links`에 행 추가:
     `{ work_id: W, chapter_id: C10, entity_id: E }`. (작가 수동 또는 AI 자동 추출)

이 시점에서 모델에는 "E는 `global_seq 3`에 `life_status=dead`가 됐다"와 "E는 `global_seq 10`인 화에 등장한다"가 **둘 다 사실로 기록**되어 있다. 두 사실의 시점 비교가 가능하다는 점이 핵심이다.

### 8.2. 탐지 (v2 충돌 감지 규칙)

> MVP는 위 8.1의 **기록·표시까지**다. 아래 자동 탐지는 **v2** 분석&피드백 기능이며, 그 규칙이 본 모델 위에서 SQL로 표현 가능함을 보이는 것이 본 절의 목적이다.

규칙(자연어): "어떤 엔티티 E의 `life_status`가 시점 `seq_dead`에 `dead`로 기록되었는데, E가 `seq_dead`보다 **이후** 화에 등장(화-엔티티 링크)하고, 그 사이에 부활/회귀류 상태(`alive`/`revived`)로 되돌리는 타임라인 상태가 **없다면** → 충돌 후보."

이를 본 모델의 테이블로 직접 질의할 수 있다(개념 SQL):

```sql
-- 사망 시점 이후 등장했고, 그 사이 부활 기록이 없는 충돌 후보
SELECT e.name, dead.chapter_id AS died_at, appear.chapter_id AS reappeared_at
FROM timeline_states dead
JOIN chapters dc       ON dc.id = dead.chapter_id
JOIN scene_entity_links appear ON appear.entity_id = dead.entity_id
JOIN chapters ac       ON ac.id = appear.chapter_id
...
WHERE dead.work_id = :work_id              -- 테넌트 격리
  ...
  AND ac.global_seq > dc.global_seq        -- 사망보다 이후 시점 등장
  AND NOT EXISTS (                          -- 그 사이 부활 기록 없음
    SELECT 1 FROM timeline_states rev
    JOIN chapters rc ON rc.id = rev.chapter_id
    WHERE rev.entity_id = dead.entity_id
      AND rev.state_key = 'life_status'
      AND rev.state_value IN ('alive','revived')
      AND rc.global_seq > dc.global_seq
      AND rc.global_seq <= ac.global_seq
  );
```

탐지가 성립하는 이유는 세 가지 모델 요소가 맞물리기 때문이다.

- **타임라인 상태**가 "사망"을 특정 시점(`chapter_id` → `global_seq`)에 묶어 기록한다 → "언제" 죽었는지 안다.
- **화-엔티티 링크**가 "등장"을 특정 시점 화에 묶어 기록한다 → "언제" 다시 나왔는지 안다.
...
```mermaid
flowchart LR
    D["timeline_states<br/>E: life_status=dead<br/>@ 3화 (global_seq=3)"] --> CMP{global_seq 비교<br/>등장 > 사망?}
    A["scene_entity_links<br/>E 등장 @ 10화<br/>(global_seq=10)"] --> CMP
    CMP -- "예 (10 > 3)" --> REV{그 사이 부활<br/>상태 기록 있나?}
...
erDiagram
    ...
    chapters ||--o{ timeline_states : "상태 확정 시점"
    chapters ||--o{ scene_entity_links : "등장"
...
## 부록: 관련 결정 (Reference)
- ADR-0002: 하이브리드 메모리(정형 엔티티 카드 + 타임라인 상태 + 벡터(pgvector) + 화-엔티티 링크). 본 문서가 그 데이터 스키마 구체화. remove-scene ADR(260716-17a)이 씬→화 단위로 개정.
...
- remove-scene ADR(260716-17a): Scene 계층 제거 — 챕터가 본문·시점·임베딩의 최소 단위로 흡수.
- 용어: `.forge/CONTEXT.md` (작품/World Bible/엔티티 카드/타임라인 상태/화/화-엔티티 링크/메모리).
